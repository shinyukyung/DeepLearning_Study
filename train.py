        ...
        with tf.Session() as sess:
            ...
            for e in range(self.Epoch):
                for batch_x, batch_y in utils.get_batch(self.X_train, self.Label, self.Batch_Size):
                    iteration += 1
                    feed = {self.Model.inputs: self.X_train, self.Model.labels: self.Label, self.Model.learning_rate: self.Learning_Rate, self.Model.is_training: True}
                    ...
                    if iteration %  self.Train_Collect == 0:
                        ...
                        if iteration % self.Train_Print == 0:
                             print("epoch: {}/{}".format(e + 1, self.Epoch), "train loss: {:.4f}".format(train_loss), "train acc: {:.4f}".format(train_acc))
